"""E2E tests for the IO and PWA domains (real browser flows).

Full end-to-end flows against a real browser, including IO lifecycle
(printToPDF → read → close), resolveBlob with a real blob, PWA flows
with suppress (no PWA installable in headless), type validation in
real browser context, and meta tests.
"""
...
class TestIOE2E:
    async def test_domain_accessible_from_session(self) -> None:
        async with (
            ...
            assert isinstance(session.io, IODomain)
    ...
    async def test_full_io_lifecycle(self) -> None:
        async with (
            await CDPClient.launch(headless=True, extra_args=["--headless=new"]) as client,
            await client.new_page() as session,
        ):
...
class TestPWAE2E:
    async def test_domain_accessible_from_session(self) -> None:
        async with (
            ...
            assert isinstance(session.pwa, PWADomain)
    ...
    async def test_type_error_change_app_user_settings_link_capturing_int(
        self,
    ) -> None:
        async with (
            await CDPClient.launch(headless=True) as client,
    ...
    async def test_type_error_change_app_user_settings_display_mode_int(
        self,
    ) -> None:
        async with (
            await CDPClient.launch(headless=True) as client,
...
@pytest.mark.e2e
class TestIOEdgeE2E:
    async def test_read_until_eof_full(self) -> None:
        async with (
            await CDPClient.launch(headless=True, extra_args=["--headless=new"]) as client,
            await client.new_page() as session,
        ):
    ...
    async def test_close_invalid_handle(self) -> None:
        async with (
            ...
            with contextlib.suppress(Exception):
                await session.io.close("invalid-handle-xyz")
    ...
    async def test_read_invalid_handle(self) -> None:
        async with (
            ...
            with contextlib.suppress(Exception):
                await session.io.read("invalid-handle-xyz")
...
class TestIOEdgeE2ERound2:
    ...
    async def test_read_base64_encoded_field(self) -> None:
        async with (
            await CDPClient.launch(headless=True, extra_args=["--headless=new"]) as client,
            await client.new_page() as session,
        ):
